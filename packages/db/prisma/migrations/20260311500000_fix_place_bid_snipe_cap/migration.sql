-- Fix: cap anti-snipe extension so total never exceeds max_snipe_extension_minutes
--
-- Previously the last extension could overshoot the cap (e.g. 29 min + 2 min = 31 min
-- when max is 30 min). Now the final extension is clamped to the remaining headroom.
